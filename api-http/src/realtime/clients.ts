...
/**
 * Per-socket state (ported from realtime-gateway). Kept in a module-level
 * registry (not on the ws object) so the publisher can look up a socket's userId
 * for per-user delivery (issues.md §8.1) and the heartbeat/JWT-recheck loop can
 * iterate every live client (§8.3/§8.4).
 */
